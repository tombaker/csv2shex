...
def test_statement_initialized_from_named_arguments_and_order_is_insignficant():
    ...
    assert Statement(
        shapeID="@photo", prop_id="dcterms:creator", value_node_type="URI"
    ) == Statement(prop_id="dcterms:creator", shapeID="@photo", value_node_type="URI")
...
def test_statement_initialized_by_assignment_with_some_none():
    ...
    assert shap == Statement(
        shapeID=None, prop_id="dcterms:creator", value_node_type="URI"
    )
...
def test_statement_bad_field_initialized_by_assignment():
    ...
    with pytest.raises(TypeError):
        assert shap == Statement(
            foobar="@photo", prop_id="dcterms:creator", value_node_type="URI"
        )
