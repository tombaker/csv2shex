...
from ShExJSG.ShExJ import Shape, IRIREF, TripleConstraint, NodeConstraint, shapeExpr, OneOf

from csv2shex.mkshapes import Shape as CSVShape, Statement as CSVStatement


# Questions:
# 1) is s.shape_id always present or do we have to supply it?   A: According to mkstatements, will always be present.
# 2) What is the shape label used for?  A: probably nothing except documentation.  Should we embed it as a comment?
# 3) Are multiple shape statements considered ShapeOr or ShapeAnd?   We are assuming AND for the time being
# 4) We assign prop_label to statement.id, but but there may be labels that don't fit the id value space
# 5) Need to define how "prefixed URI string" maps to IRIREF -- probably need a sub function
...
def statementtonodeconstraint(statement: CSVStatement) -> Optional[shapeExpr]:
    ...
    if statement.shape_ref:
        if rval:
            raise ValueError(f"Statement can't have both a NodeConstraint and a ValueConstraint")
